...
import { getSliderItems, getSliderBySlug, addSliderItem, updateSliderItem, deleteSliderItem } from '@/app/data/admin/sliderData';
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (slug) {
      ...
      const sliderItem = await getSliderBySlug(slug);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!data.title || !data.imageUrl) {
      return NextResponse.json(
        { success: false, message: 'Başlık ve görsel URL\'si gereklidir' },
        { status: 400 }
      );
    }
    
    // Yeni slider öğesi ekle
    const newSlider = await addSliderItem({
      title: data.title,
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const updatedSlider = await updateSliderItem(data.id, {
      title: data.title,
      subtitle: data.subtitle,
      description: data.description,
      imageUrl: data.imageUrl,
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Slider öğesini sil
    const deleted = await deleteSliderItem(id);
